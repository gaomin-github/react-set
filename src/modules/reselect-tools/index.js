...
export function selectorGraphSelf(selectorKey = defaultSelectorKey) {
...
const addToolName = (selector) => {
  ...
  const tool_name = (selector.dependencies || []).reduce((base, dep) => {
    // console.log('base',base,'dep',dep)
    return base + _sumString(dep);
  }, (selector.resultFunc ? selector.resultFunc : selector).toString());
...
const getToolContent = selector => {
  ...
  const str = (selector.dependencies || []).reduce((base, dep) => {
    // console.log('base',base,'dep',dep)
    return base;
  }, (selector.resultFunc ? selector.resultFunc : selector).toString());
  return str;
