...
import { updateModuleKey, updateNodeType } from '$redux/actions';
import './index.less';

const ranalyz = (state) => state.ranalyz;
const curnode_in = createSelector(ranalyz, (data) => data.In);
const curnode_out = createSelector(ranalyz, (data) => data.Out);
const curnode_content = createSelector(ranalyz, (data) => data.S_title);
const nodeType = createSelector(ranalyz, (data) => data.nodeType);
const moduleList = createSelector(ranalyz, (data) => {
...
const curNodeRefer = createSelector(ranalyz, (data) => data.curNodeRefer);
...
interface IDispatchProps {
  updateNodeType: typeof updateNodeType;
  updateModuleKey: typeof updateModuleKey;
}
...
class ReduxAnalyz extends PureComponent<IProp & IDispatchProps, stateProps> {
  ...
  private handleTypeChange(event: any) {
    ...
    updateNodeType(curVal);

    EventEmitter.emit('redux_analyz_update', curVal);
  }

  private handleModuleChange(event: any) {
    const { updateModuleKey } = this.props;
    if (!event.target) return;
    const curVal = event.target.value === 'all' ? '' : event.target.value;
    // console.log('curVal',curVal)
    updateModuleKey(curVal);
    EventEmitter.emit('redux_analyz_update', curVal);
  }
  ...
  render() {
    ...
    return (
      <div className="redux-analyz">
        <div
          className="analyz_panel"
          style={{ width: `${this.state.sliderXPosition}px` }}
        >
          <div className="analyz_panel-title">
            <div className="analyz_panel-label">title</div>
            {curnode_content && curnode_content.toString().substring(0, 100)}
          </div>
          <div className="analyz_panel-output">
            <div className="analyz_panel-label">input</div>
            {curnode_in ? (
              typeof curnode_in === 'object' ? (
                <ReactJson
                  src={curnode_in}
                  name={false}
                  collapsed={true}
          ...
          <div className="analyz_panel-input">
            <div className="analyz_panel-label">output</div>
            {curnode_out ? (
              typeof curnode_out === 'object' ? (
                <ReactJson
                  src={curnode_out}
                  name={false}
                  collapsed={true}
          ...
          <div className="analyz_panel-refer">
            ...
            <div className="refer_list">
              {curNodeRefer
                ? curNodeRefer.map((refer, index) => {
                    return (
                      <div key={refer} className="refer_list-item">
                        {refer}
                      </div>
                    );
                  })
        ...
        <div
          className="analyz_slider"
          onMouseDown={this.handleMouseDown.bind(this)}
          style={{ left: `${this.state.sliderXPosition}px` }}
        ></div>
...
export default connect(
  ...
  { updateNodeType, updateModuleKey }
)(ReduxAnalyz);
